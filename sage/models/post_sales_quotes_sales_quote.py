...
class PostSalesQuotesSalesQuote(object):
    """
    Attributes:
    ...
    openapi_types = {
        ...
        "quote_lines": "list[PostSalesEstimatesSalesEstimateEstimateLines]",
    ...
    def quote_lines(self, quote_lines):
        if self.local_vars_configuration.client_side_validation and quote_lines is None:
            raise ValueError("Invalid value for `quote_lines`, must not be `None`")
    ...
    def __eq__(self, other):
        if not isinstance(other, PostSalesQuotesSalesQuote):
            return False
