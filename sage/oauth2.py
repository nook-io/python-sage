...
class TokenApi:
    """
    Api class handles interactions with sage token API endpoints
    """
    ...
    def refresh_token(self, refresh_token):
        """
        Call sage identity API to refresh auth2 access token using refresh token
    ...
    def revoke_token(self, refresh_token):
        """
        Call sage identity API to revoke access tokens and remove all a user's connections using refresh token
        :param refresh_token: str auth2 refresh token
    ...
    def get_client_credentials_token(self, app_store_billing):
        """
        Call sage Identity API to obtain an access token via OAuth2 Client Credentails grant type
        :return: dictionary with new auth2 token
        """
        post_data = {
            "client_id": self.client_id,
