...
class TaxSchemesApi(object):
    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client

    def get_tax_schemes(self, **kwargs):
        kwargs["_return_http_data_only"] = True
        return self.get_tax_schemes_with_http_info(**kwargs)

    def get_tax_schemes_with_http_info(self, **kwargs):
        ...
        if (
            ...
            raise ApiValueError(
                "Invalid value for parameter `items_per_page` when calling `get_tax_schemes`, must be a value less than or equal to `200`"
            )
        if (
            self.api_client.client_side_validation
            and "items_per_page" in local_var_params
            ...
            raise ApiValueError(
                "Invalid value for parameter `items_per_page` when calling `get_tax_schemes`, must be a value greater than or equal to `1`"
            )
        collection_formats = {}
        path_params = {}
        query_params = []
    ...
    def get_tax_schemes_key(self, key, **kwargs):
        kwargs["_return_http_data_only"] = True
        return self.get_tax_schemes_key_with_http_info(key, **kwargs)

    def get_tax_schemes_key_with_http_info(self, key, **kwargs):
        local_var_params = locals()
        ...
        for key, val in six.iteritems(local_var_params["kwargs"]):
            if key not in all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_tax_schemes_key" % key
                )
            local_var_params[key] = val
        del local_var_params["kwargs"]
        if self.api_client.client_side_validation and (
            ...
            raise ApiValueError(
                "Missing the required parameter `key` when calling `get_tax_schemes_key`"
            )
        collection_formats = {}
        path_params = {}
