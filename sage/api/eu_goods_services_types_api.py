...
class EUGoodsServicesTypesApi(object):
    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client

    def get_eu_goods_services_types(self, **kwargs):
        kwargs["_return_http_data_only"] = True
        return self.get_eu_goods_services_types_with_http_info(**kwargs)

    def get_eu_goods_services_types_with_http_info(self, **kwargs):
        ...
        for key, val in six.iteritems(local_var_params["kwargs"]):
            if key not in all_params:
                raise ApiTypeError(
                    ...
                    " to method get_eu_goods_services_types" % key
                )
        ...
        if (
            ...
            raise ApiValueError(
                "Invalid value for parameter `items_per_page` when calling `get_eu_goods_services_types`, must be a value less than or equal to `200`"
            )
        if (
            self.api_client.client_side_validation
            and "items_per_page" in local_var_params
            ...
            raise ApiValueError(
                "Invalid value for parameter `items_per_page` when calling `get_eu_goods_services_types`, must be a value greater than or equal to `1`"
            )
        collection_formats = {}
        path_params = {}
        query_params = []
    ...
    def get_eu_goods_services_types_key(self, key, **kwargs):
        kwargs["_return_http_data_only"] = True
        return self.get_eu_goods_services_types_key_with_http_info(key, **kwargs)

    def get_eu_goods_services_types_key_with_http_info(self, key, **kwargs):
        local_var_params = locals()
        ...
        for key, val in six.iteritems(local_var_params["kwargs"]):
            if key not in all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_eu_goods_services_types_key" % key
                )
            local_var_params[key] = val
        del local_var_params["kwargs"]
        if self.api_client.client_side_validation and (
            ...
            raise ApiValueError(
                "Missing the required parameter `key` when calling `get_eu_goods_services_types_key`"
            )
        collection_formats = {}
        path_params = {}
